...
// Escucha cuando el usuario está autenticado
onAuthStateChanged(auth, async (user) => {
  ...
  // Obtener información del doctor asignado
  const doctorRef = doc(db, "doctores", data.doctorId);
  const doctorDoc = await getDoc(doctorRef);

  if (!doctorDoc.exists()) {
    alert("El doctor asignado no existe en la base de datos.");
    return;
  }

  // Guardamos el ID del doctor
  assignedDoctor = doctorDoc.id;
